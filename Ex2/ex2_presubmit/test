...
#             print (f.read())
    ...
    try:
        ...
        new_shape = 4960
        if (reduced_samples.shape[0] != new_shape and reduced_samples.shape[0] != (audio_data.shape[0] / 2)):
    ...
    print ('=== Section 3.2 ===\n')
    print ('derivative using fourier')
    sys.stdout.flush()
    try:
        magnitude = sol2.fourier_der(im)
        if not np.all(magnitude.shape==im.shape):
...
if __name__ == '__main__':
    ...
    It is your responsibility to make sure your results are actually correct and not only technically valid.""")
